    ...
    return <>
      ...
      <Form name="form1" layout={"horizontal"}>
        ...
        <Form.Item label={"请输入B点"} name={"max"}>
          <Input placeholder={"请输入值"} value={this.state.max} inputMode={"decimal"} clearable={true} onlyShowClearWhenFocus={false} onChange={(max) => this.setState({ max: parseFloat(max.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')) })} />
        </Form.Item>
      </Form>
