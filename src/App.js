    ...
    return <>
      ...
      <Form name="form1" layout={"horizontal"}>
        ...
        <Form.Item label={"请输入A点"} name={"min"}>
          <Input placeholder={"请输入值"} clearable onChange={(min) => this.setState({ min: parseFloat(min) })} />
        </Form.Item>
        <Form.Item label={"请输入B点"} name={"max"}>
          <Input placeholder={"请输入值"} clearable onChange={(max) => this.setState({ max: parseFloat(max) })} />
        </Form.Item>
      </Form>
