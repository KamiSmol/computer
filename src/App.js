    ...
    return <>
      ...
      <Form name="form1" layout={"horizontal"}>
        ...
        <Form.Item label={"请输入A点"} name={"min"}>
          <Input placeholder={"请输入值"} inputMode={"decimal"} clearable onlyShowClearWhenFocus={false} onChange={(min) => this.setState({ min: parseFloat(min.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')) })} />
        </Form.Item>
        <Form.Item label={"请输入B点"} name={"max"}>
          <Input placeholder={"请输入值"} inputMode={"decimal"} clearable onlyShowClearWhenFocus={false} onChange={(max) => this.setState({ max: parseFloat(max.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')) })} />
        </Form.Item>
      </Form>
